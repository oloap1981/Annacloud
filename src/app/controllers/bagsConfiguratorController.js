angular
  ...
  .controller("unadunaConfiguratorController2", function (
    $scope,
    listeService,
    loginService,
    carrelloService,
    ...
    $scope.categorieTracolle = [
      "TRACOLLA_BASE",
      "TRACOLLA_CLASSICA-DOT",
      "TRACOLLA_CLASSICA-STELLE",
      "TRACOLLA_CATENELLA",
      "TRACOLLA_INCROCIATA",
      "TRACOLLA_CENTRALE",
    ];
    ...
    $scope.baseUrlSymbols =
      "https://s3.eu-central-1.amazonaws.com/unaduna-images-bucket/MODELLI/MODELLO/INIZIALI/";
    $scope.baseUrlThumbCategorieTracolle =
      "https://s3.eu-central-1.amazonaws.com/unaduna-images-bucket/MODELLI/MODELLO/TRACOLLE/THUMBNAILS/";
    $scope.baseUrlThumbCategorieCiondoli =
      "https://s3.eu-central-1.amazonaws.com/unaduna-images-bucket/MODELLI/MODELLO/CIONDOLI/THUMBNAILS/";
    ...
    configController.getModelloSelezionato = function () {
      if ($scope.modelloSelezionato != "") {
        var nomeModello =
          $scope.modelloSelezionato.charAt(0).toUpperCase() +
          $scope.modelloSelezionato.slice(1);
    ...
    configController.getThumbnailNameTracolle = function (entita) {
      if (entita) {
        var toReplace = configController.modelNameTranslate(
          $scope.modelloSelezionato
        );
        if (toReplace != undefined) {
          var thumbnailName =
            $scope.baseUrlThumbCategorieTracolle.replace("MODELLO", toReplace) +
            "THUMBNAIL_" +
            entita.replace("-", "_") +
            ".png";
    ...
    configController.getThumbnailNameCiondoli = function (entita) {
      if (entita) {
        ...
        if (toReplace != undefined) {
          var thumbnailName =
            $scope.baseUrlThumbCategorieCiondoli.replace("MODELLO", toReplace) +
            "THUMBNAIL_" +
            entita.replace("-", "_") +
            ".png";
          return thumbnailName;
    ...
    configController.selezionaCategoriaTracolla = function (entita) {
      ...
      $scope.variantiTracolle = configController.ordinaEntita(
        $scope.variantiTracolle
      );
    ...
    configController.selezionaCategoriaCiondolo = function (entita) {
      ...
      $scope.variantiCiondoli = configController.ordinaEntita(
        $scope.variantiCiondoli
      );
    ...
    configController.selezioneTipoAccessorio = function (tipoAccessorio) {
      ...
      if (tipoAccessorio == "tracolle") {
        ...
        if ($scope.tipoTracollaSelezionata == "") {
          ...
          configController.selezionaCategoriaTracolla(
            $scope.tipoTracollaSelezionata
          );
        }
        ...
        if ($scope.tipoCiondoloSelezionato == "") {
          ...
          configController.selezionaCategoriaCiondolo(
            $scope.tipoCiondoloSelezionato
          );
        }
      } else {
        ...
        for (var i = 0; i < $scope.modelli.length; i++) {
          ...
          if (modello.nome == $scope.modelloSelezionato) {
            for (var j = 0; j < $scope.entita.length; j++) {
              var entitaSingola = $scope.entita[j];
              if (
                (entitaSingola.categoria == tipoAccessorio) &
                (entitaSingola.modello == $scope.modelloSelezionato)
              ) {
                if (entitaSingola.vincoloColore == true) {
                  if (entitaSingola.categoria == "stile") {
                    $scope.mapEmboss.set(
                      entitaSingola.nomeStile + "_" + entitaSingola.colore,
                      entitaSingola
                    );
                  }
                  ...
                  if (entitaSingola.categoria == "tracolle") {
                    ...
                    $scope.mapMetalloBorchie.set(
                      entitaSingola.nomeBorchia + "_" + entitaSingola.metallo,
                      entitaSingola
                    );
                  }
        ...
        if ($scope.tipoEntitaSelezionata != "iniziali") {
          $scope.entitaTipoAccessorioSelezionato = configController.ordinaEntita(
            $scope.entitaTipoAccessorioSelezionato
          );
        } else {
          $scope.entitaTipoAccessorioSelezionato = [];
    ...
    configController.ordinaEntita = function (entitaNonOrdinate) {
      ...
      for (var i = 0; i < size; i++) {
        if (
          entitaNonOrdinate[i].ordineInterfaccia == null ||
          entitaNonOrdinate[i].ordineInterfaccia == "" ||
          entitaNonOrdinate[i].ordineInterfaccia == 0
        ) {
          return entitaNonOrdinate;
        }
        tempEntita[entitaNonOrdinate[i].ordineInterfaccia - 1] =
          entitaNonOrdinate[i];
      }
      ...
      for (var i = 0; i < $scope.entita.length; i++) {
        ...
        if (entita.categoria == "ciondoli") {
          ...
          if (entita.metallo == "oro") {
            ...
            if (
              $scope.modelliCiondoliOro
                .get(categoriaCiondolo)
                .indexOf(modelName) == -1
            ) {
      ...
      for (var i = 0; i < $scope.entita.length; i++) {
        ...
        if (entita.categoria == "tracolle") {
          ...
          if (entita.metallo == "oro") {
            ...
            if (
              $scope.modelliTracolleOro
                .get(categoriaTracolla)
                .indexOf(modelName) == -1
            ) {
    ...
    configController.scegliModello = function (modello) {
      ...
      listeService.getAccessori(modello.nome).then(function (res2) {
        ...
        var entitaModello = configController.getInternalEntitaObjct(
          "modello",
          modello.codice,
          modello.nome,
          modello.nome,
          ...
          "",
          modello.accessori,
          modello.accessorioInizialeSelezionato
        );
        ...
        $scope.tipiAccessoriModelloSelezionato = $scope.tipiAccessori.get(
          modello.nome
        );

        if (
          configController.singolaEntitaPresente(
            "metalleria",
            modello.nome,
            "metalleria_argento"
          )
        ) {
          var singolaEntita = configController.getSingolaEntita(
            "metalleria",
            modello.nome,
            "metalleria_argento"
          );
          var entitaMetalleria = configController.getInternalEntitaObjct(
            singolaEntita.categoria,
            singolaEntita.codice,
            singolaEntita.nome,
            singolaEntita.nome,
            singolaEntita.prezzo,
            0,
            singolaEntita.categoria,
            singolaEntita.urlStripe,
            3,
            singolaEntita.urlStripe,
            singolaEntita.nomeStile,
            singolaEntita.nomeBorchia,
            singolaEntita.colore,
            singolaEntita.metallo,
            [],
            ""
          );
          $scope.metalleriaObbligatoria = configController.getUrlMetalleria(
            modello.nome,
            "argento"
          );
        ...
        if (modello.nome == "bucket_paglia") {
          var singolaEntitaFoderaPaglia = configController.getSingolaEntita(
            "fodere",
            modello.nome,
            "FODERA_RES_NERO"
          );
          var entitaFoderaPaglia = configController.getInternalEntitaObjct(
            singolaEntitaFoderaPaglia.categoria,
            singolaEntitaFoderaPaglia.codice,
            singolaEntitaFoderaPaglia.nome,
            singolaEntitaFoderaPaglia.nome,
            singolaEntitaFoderaPaglia.prezzo,
            0,
            singolaEntitaFoderaPaglia.categoria,
            singolaEntitaFoderaPaglia.urlStripe,
            6,
            singolaEntitaFoderaPaglia.urlStripe,
            singolaEntitaFoderaPaglia.nomeStile,
            singolaEntitaFoderaPaglia.nomeBorchia,
            singolaEntitaFoderaPaglia.colore,
            singolaEntitaFoderaPaglia.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaFoderaPaglia.urlStripe,
            6,
            false,
            entitaFoderaPaglia
          );
        ...
        if (modello.nome == "circle") {
          $scope.coloreSelezionato = "almond";
          //creo un set di default di accessori: corda, strozzino, ciondolo (,tracolla?)
          var singolaEntitaColoreCircle = configController.getSingolaEntita(
            "colore",
            modello.nome,
            "CORPO_RES_ALMOND"
          );
          var entitaColoreCircle = configController.getInternalEntitaObjct(
            singolaEntitaColoreCircle.categoria,
            singolaEntitaColoreCircle.codice,
            singolaEntitaColoreCircle.nome,
            singolaEntitaColoreCircle.nome,
            singolaEntitaColoreCircle.prezzo,
            0,
            singolaEntitaColoreCircle.categoria,
            singolaEntitaColoreCircle.urlStripe,
            5,
            singolaEntitaColoreCircle.urlStripe,
            singolaEntitaColoreCircle.nomeStile,
            singolaEntitaColoreCircle.nomeBorchia,
            singolaEntitaColoreCircle.colore,
            singolaEntitaColoreCircle.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaColoreCircle.urlStripe,
            5,
            false,
            entitaColoreCircle
          );

          var singolaEntitaCorda = configController.getSingolaEntita(
            "corde",
            modello.nome,
            "CORDA_RES_ALMOND"
          );
          var entitaCorda = configController.getInternalEntitaObjct(
            singolaEntitaCorda.categoria,
            singolaEntitaCorda.codice,
            singolaEntitaCorda.nome,
            singolaEntitaCorda.nome,
            singolaEntitaCorda.prezzo,
            0,
            singolaEntitaCorda.categoria,
            singolaEntitaCorda.urlStripe,
            6,
            singolaEntitaCorda.urlStripe,
            singolaEntitaCorda.nomeStile,
            singolaEntitaCorda.nomeBorchia,
            singolaEntitaCorda.colore,
            singolaEntitaCorda.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaCorda.urlStripe,
            6,
            false,
            entitaCorda
          );

          var singolaEntitaCiondolo = configController.getSingolaEntita(
            "ciondolo",
            modello.nome,
            "CIONDOLO_RES_ALMOND"
          );
          var entitaCiondolo = configController.getInternalEntitaObjct(
            singolaEntitaCiondolo.categoria,
            singolaEntitaCiondolo.codice,
            singolaEntitaCiondolo.nome,
            singolaEntitaCiondolo.nome,
            singolaEntitaCiondolo.prezzo,
            0,
            singolaEntitaCiondolo.categoria,
            singolaEntitaCiondolo.urlStripe,
            7,
            singolaEntitaCiondolo.urlStripe,
            singolaEntitaCiondolo.nomeStile,
            singolaEntitaCiondolo.nomeBorchia,
            singolaEntitaCiondolo.colore,
            singolaEntitaCiondolo.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaCiondolo.urlStripe,
            7,
            false,
            entitaCiondolo
          );

          var singolaEntitaStrozzino = configController.getSingolaEntita(
            "strozzino",
            modello.nome,
            "STROZZINO_RES_ALMOND"
          );
          var entitaStrozzino = configController.getInternalEntitaObjct(
            singolaEntitaStrozzino.categoria,
            singolaEntitaStrozzino.codice,
            singolaEntitaStrozzino.nome,
            singolaEntitaStrozzino.nome,
            singolaEntitaStrozzino.prezzo,
            0,
            singolaEntitaStrozzino.categoria,
            singolaEntitaStrozzino.urlStripe,
            8,
            singolaEntitaStrozzino.urlStripe,
            singolaEntitaStrozzino.nomeStile,
            singolaEntitaStrozzino.nomeBorchia,
            singolaEntitaStrozzino.colore,
            singolaEntitaStrozzino.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaStrozzino.urlStripe,
            8,
            false,
            entitaStrozzino
          );
          ...
          var entitaFodera = configController.getInternalEntitaObjct(
            singolaEntitaFodera.categoria,
            singolaEntitaFodera.codice,
            singolaEntitaFodera.nome,
            singolaEntitaFodera.nome,
            singolaEntitaFodera.prezzo,
            0,
            singolaEntitaFodera.categoria,
            singolaEntitaFodera.urlStripe,
            10,
            singolaEntitaFodera.urlStripe,
            singolaEntitaFodera.nomeStile,
            singolaEntitaFodera.nomeBorchia,
            singolaEntitaFodera.colore,
            singolaEntitaFodera.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaFodera.urlStripe,
            10,
            false,
            entitaFodera
          );
*/
          ...
          var entitaTracolla = configController.getInternalEntitaObjct(
            singolaEntitaTracolla.categoria,
            singolaEntitaTracolla.codice,
            singolaEntitaTracolla.nome,
            singolaEntitaTracolla.nome,
            singolaEntitaTracolla.prezzo,
            0,
            singolaEntitaTracolla.categoria,
            singolaEntitaTracolla.urlStripe,
            11,
            singolaEntitaTracolla.urlStripe,
            singolaEntitaTracolla.nomeStile,
            singolaEntitaTracolla.nomeBorchia,
            singolaEntitaStrozzino.colore,
            singolaEntitaTracolla.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaTracolla.urlStripe,
            11,
            false,
            entitaTracolla
          );
        ...
        if (modello.nome == "exagon") {
          $scope.coloreSelezionato = "almond";
          //creo un set di default di accessori: corda, strozzino, ciondolo (,tracolla?)

          // COLORE
          var singolaEntitaColoreExagon = configController.getSingolaEntita(
            "colore",
            modello.nome,
            "CORPO_RES_ALMOND"
          );
          var entitaColoreExagon = configController.getInternalEntitaObjct(
            singolaEntitaColoreExagon.categoria,
            singolaEntitaColoreExagon.codice,
            singolaEntitaColoreExagon.nome,
            singolaEntitaColoreExagon.nome,
            singolaEntitaColoreExagon.prezzo,
            0,
            singolaEntitaColoreExagon.categoria,
            singolaEntitaColoreExagon.urlStripe,
            5,
            singolaEntitaColoreExagon.urlStripe,
            singolaEntitaColoreExagon.nomeStile,
            singolaEntitaColoreExagon.nomeBorchia,
            singolaEntitaColoreExagon.colore,
            singolaEntitaColoreExagon.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaColoreExagon.urlStripe,
            5,
            false,
            entitaColoreExagon
          );
          ...
          // PATTINE
          var singolaEntitaPattinaExagon = configController.getSingolaEntita(
            "pattina",
            modello.nome,
            "PATTINA_RES_ALMOND"
          );
          var entitaPattinaExagon = configController.getInternalEntitaObjct(
            singolaEntitaPattinaExagon.categoria,
            singolaEntitaPattinaExagon.codice,
            singolaEntitaPattinaExagon.nome,
            singolaEntitaPattinaExagon.nome,
            singolaEntitaPattinaExagon.prezzo,
            0,
            singolaEntitaPattinaExagon.categoria,
            singolaEntitaPattinaExagon.urlStripe,
            6,
            singolaEntitaPattinaExagon.urlStripe,
            singolaEntitaPattinaExagon.nomeStile,
            singolaEntitaPattinaExagon.nomeBorchia,
            singolaEntitaPattinaExagon.colore,
            singolaEntitaPattinaExagon.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaPattinaExagon.urlStripe,
            6,
            false,
            entitaPattinaExagon
          );
          ...
          // LATERALI
          var singolaEntitaLateraliExagon = configController.getSingolaEntita(
            "laterali",
            modello.nome,
            "LATERALI_RES_ALMOND"
          );
          var entitaLateraliExagon = configController.getInternalEntitaObjct(
            singolaEntitaLateraliExagon.categoria,
            singolaEntitaLateraliExagon.codice,
            singolaEntitaLateraliExagon.nome,
            singolaEntitaLateraliExagon.nome,
            singolaEntitaLateraliExagon.prezzo,
            0,
            singolaEntitaLateraliExagon.categoria,
            singolaEntitaLateraliExagon.urlStripe,
            6,
            singolaEntitaLateraliExagon.urlStripe,
            singolaEntitaLateraliExagon.nomeStile,
            singolaEntitaLateraliExagon.nomeBorchia,
            singolaEntitaLateraliExagon.colore,
            singolaEntitaLateraliExagon.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaLateraliExagon.urlStripe,
            7,
            false,
            entitaLateraliExagon
          );

          // TRACOLLE
          var singolaEntitaTracollaExagon = configController.getSingolaEntita(
            "tracolla",
            modello.nome,
            "TRACOLLA_RES_ALMOND"
          );
          var entitaTracollaExagon = configController.getInternalEntitaObjct(
            singolaEntitaTracollaExagon.categoria,
            singolaEntitaTracollaExagon.codice,
            singolaEntitaTracollaExagon.nome,
            singolaEntitaTracollaExagon.nome,
            singolaEntitaTracollaExagon.prezzo,
            0,
            singolaEntitaTracollaExagon.categoria,
            singolaEntitaTracollaExagon.urlStripe,
            6,
            singolaEntitaTracollaExagon.urlStripe,
            singolaEntitaTracollaExagon.nomeStile,
            singolaEntitaTracollaExagon.nomeBorchia,
            singolaEntitaTracollaExagon.colore,
            singolaEntitaTracollaExagon.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
        ...
        if (modello.nome == "triangle") {
          $scope.coloreSelezionato = "almond";
          //creo un set di default di accessori: corda, strozzino, ciondolo (,tracolla?)

          // COLORE
          var singolaEntitaColoreTriangle = configController.getSingolaEntita(
            "colore",
            modello.nome,
            "CORPO_RES_ALMOND"
          );
          var entitaColoreTriangle = configController.getInternalEntitaObjct(
            singolaEntitaColoreTriangle.categoria,
            singolaEntitaColoreTriangle.codice,
            singolaEntitaColoreTriangle.nome,
            singolaEntitaColoreTriangle.nome,
            singolaEntitaColoreTriangle.prezzo,
            0,
            singolaEntitaColoreTriangle.categoria,
            singolaEntitaColoreTriangle.urlStripe,
            5,
            singolaEntitaColoreTriangle.urlStripe,
            singolaEntitaColoreTriangle.nomeStile,
            singolaEntitaColoreTriangle.nomeBorchia,
            singolaEntitaColoreTriangle.colore,
            singolaEntitaColoreTriangle.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaColoreTriangle.urlStripe,
            5,
            false,
            entitaColoreTriangle
          );

          // PATTINE
          var singolaEntitaPattinaTriangle = configController.getSingolaEntita(
            "pattina",
            modello.nome,
            "PATTINA_RES_ALMOND"
          );
          var entitaPattinaTriangle = configController.getInternalEntitaObjct(
            singolaEntitaPattinaTriangle.categoria,
            singolaEntitaPattinaTriangle.codice,
            singolaEntitaPattinaTriangle.nome,
            singolaEntitaPattinaTriangle.nome,
            singolaEntitaPattinaTriangle.prezzo,
            0,
            singolaEntitaPattinaTriangle.categoria,
            singolaEntitaPattinaTriangle.urlStripe,
            6,
            singolaEntitaPattinaTriangle.urlStripe,
            singolaEntitaPattinaTriangle.nomeStile,
            singolaEntitaPattinaTriangle.nomeBorchia,
            singolaEntitaPattinaTriangle.colore,
            singolaEntitaPattinaTriangle.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaPattinaTriangle.urlStripe,
            6,
            false,
            entitaPattinaTriangle
          );

          // LATERALI
          var singolaEntitaLateraliTriangle = configController.getSingolaEntita(
            "laterali",
            modello.nome,
            "LATERALI_RES_ALMOND"
          );
          var entitaLateraliTriangle = configController.getInternalEntitaObjct(
            singolaEntitaLateraliTriangle.categoria,
            singolaEntitaLateraliTriangle.codice,
            singolaEntitaLateraliTriangle.nome,
            singolaEntitaLateraliTriangle.nome,
            singolaEntitaLateraliTriangle.prezzo,
            0,
            singolaEntitaLateraliTriangle.categoria,
            singolaEntitaLateraliTriangle.urlStripe,
            6,
            singolaEntitaLateraliTriangle.urlStripe,
            singolaEntitaLateraliTriangle.nomeStile,
            singolaEntitaLateraliTriangle.nomeBorchia,
            singolaEntitaLateraliTriangle.colore,
            singolaEntitaLateraliTriangle.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
            entitaLateraliTriangle.urlStripe,
            7,
            false,
            entitaLateraliTriangle
          );

          // MANIGLIE
          var singolaEntitaManigliaTriangle = configController.getSingolaEntita(
            "maniglia",
            modello.nome,
            "MANIGLIA_RES_ALMOND"
          );
          var entitaManigliaTriangle = configController.getInternalEntitaObjct(
            singolaEntitaManigliaTriangle.categoria,
            singolaEntitaManigliaTriangle.codice,
            singolaEntitaManigliaTriangle.nome,
            singolaEntitaManigliaTriangle.nome,
            singolaEntitaManigliaTriangle.prezzo,
            0,
            singolaEntitaManigliaTriangle.categoria,
            singolaEntitaManigliaTriangle.urlStripe,
            6,
            singolaEntitaManigliaTriangle.urlStripe,
            singolaEntitaManigliaTriangle.nomeStile,
            singolaEntitaManigliaTriangle.nomeBorchia,
            singolaEntitaManigliaTriangle.colore,
            singolaEntitaManigliaTriangle.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
          ...
          // TRACOLLE
          var singolaEntitaTracollaTriangle = configController.getSingolaEntita(
            "tracolla",
            modello.nome,
            "TRACOLLA_RES_ALMOND"
          );
          var entitaTracollaTriangle = configController.getInternalEntitaObjct(
            singolaEntitaTracollaTriangle.categoria,
            singolaEntitaTracollaTriangle.codice,
            singolaEntitaTracollaTriangle.nome,
            singolaEntitaTracollaTriangle.nome,
            singolaEntitaTracollaTriangle.prezzo,
            0,
            singolaEntitaTracollaTriangle.categoria,
            singolaEntitaTracollaTriangle.urlStripe,
            7,
            singolaEntitaTracollaTriangle.urlStripe,
            singolaEntitaTracollaTriangle.nomeStile,
            singolaEntitaTracollaTriangle.nomeBorchia,
            singolaEntitaTracollaTriangle.colore,
            singolaEntitaTracollaTriangle.metallo,
            [],
            ""
          );
          configController.aggiungiElementoAStack(
        ...
        configController.selezioneTipoAccessorio(
          modello.accessorioInizialeSelezionato
        );
    ...
    configController.caricaAccessoriPerModello = function (modello) {
      ...
      listeService.getAccessori(modello.nome).then(function (res2) {
        ...
        $scope.coloreSelezionato = configController.getColoreVincolante(
          $scope.configurazione
        );
        ...
        configController.aggiungiElementoAStackNoConfig(
          $scope.metalleriaObbligatoria,
          3,
          false
        );

        $scope.coloreVincolante = configController.getColoreVincolante(
          $scope.configurazione
        ); //scelgo il nero come colore vincolante di default
        $scope.metalloVincolante = configController.getMetalloVincolante(
          $scope.configurazione
        );
    ...
    configController.getInternalEntitaObjct = function (
      categoria,
      codice,
      descrizione,
      nome,
    ...
    configController.addEntitaToConfigurazione = function (
      categoria,
      codice,
      descrizione,
      nome,
      ...
      var entita = configController.getInternalEntitaObjct(
        categoria,
        codice,
        descrizione,
        nome,
    ...
    configController.isSelected = function (entita) {
      if (entita != undefined) {
        if (entita.nome != undefined && entita.nome != "") {
          return (
            configController.normalizzaStringheMetallo(entita.nome) ===
            $scope.mappaEntitaSelezionate[$scope.tipoEntitaSelezionata]
          );
        }
      }
      return false;
    ...
    configController.ricaricaEntita = function (entita) {
      ...
      if (entita.categoria == "iniziali") {
        ...
        if (
          entita.nome ==
            $scope.mappaEntitaSelezionate[$scope.tipoEntitaSelezionata] &&
          $scope.tipoEntitaSelezionata != "colore" &&
          $scope.tipoEntitaSelezionata != "metalleria"
        ) {
        ...
        if ($scope.tipoEntitaSelezionata.startsWith("colore")) {
          ...
          if ($scope.embossSelezionato) {
            ...
            var embossUrl = $scope.mapEmboss.get(
              $scope.nomeStileSelezionato + "_" + entita.colore
            );
        ...
        if ($scope.tipoEntitaSelezionata.startsWith("metalleria")) {
          if ($scope.tracollaSelezionata) {
            var chiaveT = configController.normalizzaNomeConMetallo(
              $scope.nomeTracollaSelezionata,
              entita.metallo
            );
          ...
          if ($scope.ciondoloSelezionata) {
            var chiaveC = configController.normalizzaNomeConMetallo(
              $scope.nomeCiondoloSelezionato,
              entita.metallo
            );
          ...
          if ($scope.borchieSelezionate) {
            var borchieUrl = $scope.mapMetalloBorchie.get(
              $scope.nomeBorchiaSelezionata + "_" + entita.metallo
            );
        ...
        configController.aggiungiElementoAStackNoConfig(
          url,
          entita.ordine,
          entita.categoria != "colore" && entita.categoria != "metalleria"
        );
        ...
        if ($scope.tipoEntitaSelezionata == "stile") {
          if ($scope.stack.indexOf(url) == -1) {
            ...
            $scope.embossSelezionato =
              $scope.stack[entita.ordine] != undefined &&
              $scope.stack[entita.ordine] != null;
          }
    ...
    configController.selezionaEntita = function (entita) {
      ...
      if (
        ...
        $scope.nomeEntitaSelezionata = configController.normalizzaStringheMetallo(
          entita.nome
        );
        $scope.mappaEntitaSelezionate[
          $scope.tipoEntitaSelezionata
        ] = configController.normalizzaStringheMetallo(entita.nome);
        //inserisco nella mappa
      }

      html2canvas(document.querySelector("#spritespin"), { async: false }).then(
        function (canvas) {
      ...
      configController.aggiungiStrato(
        url,
        entita.ordine,
        entita.categoria != "colore" &&
    ...
    configController.aggiungiStrato = function (
      strato,
      ordine,
      eliminabile,
      entita
    ...
    configController.aggiungiElementoAStack = function (
      strato,
      ordine,
      eliminabile,
      entita
      ...
      if (indice == -1) {
        //lo strato non è nello stack

        var lastIndex = $scope.stack.length - 1;
        if (ordine == lastIndex + 1) {
          //metto lo strato in coda
          $scope.stack.push(strato);
        } else if (ordine > lastIndex + 1) {
          //metto stringhe vuote fino all'indice dello strato da inserire
          for (var i = lastIndex + 1; i <= ordine - 1; i++) {
            $scope.stack[i] = "";
          }
          $scope.stack.push(strato);
        } else {
          //sostituisco lo strato esistente nello stack all'indice dello strato da inserire
          $scope.stack[ordine] = strato;
          configController.removeEntitaToConfigurazione(
            entita.categoria,
            entita.nome
          );
        }
        var accessorioInizialeSelezionato =
          entita.categoria == "modello" ? entita.accessorioInizialeSelezionato : "";
        configController.addEntitaToConfigurazione(
          entita.categoria,
          entita.codice,
          entita.nome,
          entita.nome,
    ...
    configController.aggiungiElementoAStackNoConfig = function (
      strato,
      ordine,
      eliminabile
    ) {
    ...
    configController.pulisciStack = function () {
      ...
      for (var i = 0; i < $scope.stack.length; i++) {
        if ($scope.stack[i] != "") {
          //gestisco la risoluzione appena prima di passare le immagini alla merge
          url = $scope.stack[i].replace(
            "RES",
            configController.getResolutionPlaceHolder()
          );
          tempStack.push(url);
        }
      }
    ...
    configController.caricaZoom = function (frame) {
      ...
      var higherResolutionStack = configController.getHighResolutionStack(
        cleanStack
      );
      ...
      mergeImages(higherResolutionStack).then(function (b64) {
        configController.getCutImage(
          b64,
          frame,
          configController.getHigherResolution()
        );
      });
    };
    ...
    configController.caricaSpinner = function () {
      ...
      if (
        $("#spritespin") != undefined &&
        $("#spritespin").data("spritespin") != undefined
      ) {
        $scope.dataUrl = $("#spritespin")
          .data("spritespin")
          .canvas[0].toDataURL();
      }
    ...
    configController.getUrlMetalleria = function (modello, metallo) {
      for (var i = 0; i < $scope.entita.length; i++) {
        ...
        if (
          singolaEntita.modello == modello &&
          singolaEntita.metallo == metallo &&
          singolaEntita.categoria == "metalleria"
        ) {
    ...
    configController.singolaEntitaPresente = function (
      categoria,
      modello,
      nome
    ) {
      for (var i = 0; i < $scope.entita.length; i++) {
        ...
        if (
          singolaEntita.modello == modello &&
          singolaEntita.categoria == categoria &&
          singolaEntita.nome == nome
        ) {
          return true;
        }
      }
    ...
    configController.generateSymbolStack = function () {
      ...
      if (configController.areSymbolsSelected()) {
        ...
        if (symbolNumber > 0) {
          for (
            var i = 0;
            i < symbolConfigurations[symbolNumber + 1].length;
            i++
          ) {
    ...
    configController.generateArray = function () {
      ...
      if (charArraySize > 0) {
        for (
          var i = 0;
          i < $scope.symbolConfigurations[charArraySize - 1].length;
          i++
        ) {
          ...
          url = url.replace(
            "MODELLO",
            configController.modelNameTranslate($scope.modelloSelezionato)
          );
          $scope.symbolsUrlStack.push(url);
        }
      }
    ...
    configController.getSymbolUrl = function (translatedSymbol, posizione) {
      return (
        ...
        configController.getResolutionPlaceHolder() +
        "_" +
        configController.colorTranslate($scope.coloreVincolante) +
        ".png"
      );
    };
    ...
    configController.ordinaModelli = function (modelliNonOrdinati) {
      ...
      for (var i = 0; i < size; i++) {
        tempModelli[modelliNonOrdinati[i].ordineInterfaccia - 1] =
          modelliNonOrdinati[i];
      }
    ...
    configController.salvaConfigurazione = function (isCarrello) {
      ...
      var lowerResolutionStack = configController.getLowerResolutionStack(
        cleanStack
      );
      ...
      mergeImages(lowerResolutionStack).then(function (imgBase64) {
        image.src = imgBase64;
        image.onload = function () {
          //quando la thumbnail è pronta procedo all'invio al server - controllare le dimensioni della thumbnail
          ctx.drawImage(
            image,
            frameNumber * resolution,
            0,
            resolution,
            resolution,
            0,
          ...
          listeService.saveImage(base64Image, filename).then(function (res2) {
            if (res2.errorMessage != null && res2.errorMessage != "") {
              console.log(res2.errorMessage);
              logService.saveLog(
                dataLog.toISOString(),
                confUser.email,
                "bagsConfiguratorController - salvaConfigurazione - saveImage",
                "si è verificato un problema nel salvataggio dell'immagine di thumbnail per la configurazione " +
              ...
              $scope.openMessageModal(
                "C'è stato un problema nel salvataggio dell immagine su S3"
              );
              $scope.hideLoader();
            } else {
              ...
              $scope.salvaOAcquista(
                configDaSalvare.nome,
                isCarrello,
                $scope.askForName
              );
              $scope.askForName = false;
    ...
    configController.addConfigurazioneACarrelloCookies = function () {
      ...
      $scope.configurazione.taglia =
        configController.taglieManager.tagliaSelezionata;

      $scope.configurazione.elencoEntita = elencoEntita;

      var configurazioneNormalizzata = configController.normalizeConfig(
        $scope.configurazione
      );
    ...
    configController.assegnaUtenteAConfigurazione = function () {
      ...
      if (userInSession != null) {
        ...
        var idToken = jwtHelper.decodeToken(
          userInSession.signInUserSession.idToken.jwtToken
        );
    ...
    configController.normalizeConfig = function (config) {
      ...
      for (var i = 0; i < elencoEntitaDaNormalizzare.length; i++) {
        ...
        urlDaNormalizzare = urlDaNormalizzare.replace(
          "MODELLO",
          configController.modelNameTranslate($scope.modelloSelezionato)
        );
        ...
        urlStripeDaNormalizzare = urlStripeDaNormalizzare.replace(
          "1920",
          "RES"
        );
        urlStripeDaNormalizzare = urlStripeDaNormalizzare.replace(
    ...
    configController.initConfiguratore = function () {
      ...
      if (
        $scope.configurazioneId != null &&
        $scope.configurazioneId != undefined &&
        $scope.configurazioneId != ""
      ) {
        // l'id c'è, provo a caricare la configurazione
        listeService
          .getConfigurazione($scope.configurazioneId)
          .then(function (res2) {
            var configurazione = res2.data.configurazione;
            if (configurazione != undefined) {
              var nomeModello = configController.getNomeModelloConfigurazione(
                configurazione
              );
    ...
    configController.openZoom = function () {
      ...
      html2canvas(document.querySelector("#spritespin")).then(function (
        canvas
      ) {
        //$("#loader").show();
        $("#loader").fadeIn("slow");
